...
- Update: Users are able to edit a searched movie that has been added in the databse using the PATCH "/edit" endpoint (requires request body with "title" (string), "description" (string), "year" (integer), "duration" in minutes (integer) and 1-5 "rating" (integer))
- Delete: Users are able to delete a searched movie that has been added in the databse using the DELETE "/delete/:id" endpoint

## Architecture
The architecture pattern applied in this project was RESTful, essentially. It can be expanded using CLEAN architecture pattern if necessary.
